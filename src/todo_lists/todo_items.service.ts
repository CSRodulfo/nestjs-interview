...
export class TodoItemsService {
  ...
  async create(todoListId: number, dto: CreateTodoItemDto): Promise<TodoItem> {
    const todoItem = this.todoItemRepository.create({ ...dto, todoListId });
    return await this.todoItemRepository.save(todoItem);
  }

  async update(todoListId: number, id: number, dto: UpdateTodoItemDto): Promise<TodoItem> {
    return await this.todoItemRepository.save({ id, todoListId, ...dto } as TodoItem);
  }
